...
function uniqueFunc1() {
    ...
    user.forEach(el => {
        ...
        //т.к includes плохо видит вложенные объекты, то приводим el к string(JSON.stringify)
...
//1.(new Set(users.map((item) =>JSON.stringify(item))))- приведение в string значений из users и педача в Set(не массив)
